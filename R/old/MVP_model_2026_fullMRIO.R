mvp.model.fullMRIO <- function(t, y, parms, A.mat, B.mat) {
  ...
  A.xr.matrix <- cbind(
    rbind(
      matrix(1, ncol = K, nrow = K), # Creates a 2N x 2N matrix of same dimensions of A
      matrix(sim['Z2_xr', 1], ncol = K, nrow = K)
    ), # Four block matrices: clockwise, 1s, xr2, xr1, and 1s
    rbind(
      matrix(sim['Z1_xr', 1], ncol = K, nrow = K),
  ...
  sim[z.lab('c'), i] <- parms[z.lab('alpha1')] *
    sim[z.lab('ydw'), i] +
    parms[z.lab('alpha2')] * sim[z.lab('ydc'), i] +
    parms[z.lab('alpha3')] * sim[z.lab('v'), i - 1]
  if (t > 2) {
    sim[z.lab('c'), i] <- sim[z.lab('c'), i] / sim[z.lab('pa'), i - 1]
  }

  # Disposable Income of Domestic Households
  sim[z.lab('yd'), i] <- sim[z.lab('wb'), i] -
    sim[z.lab('t'), i] +
    sim[z.lab('f_b'), i] +
    sim[z.lab('rb'), i - 1] * sim[paste0(z.lab('b_s_'), zlabs), i - 1] +
    sim[rev(z.lab('rb')), i - 1] *
      sim[paste0(z.lab('b_s_'), rev(zlabs)), i - 1] +
    sim[z.lab('rm'), i - 1] * sim[z.lab('mh'), i - 1] -
    sim[z.lab('rh'), i - 1] * sim[z.lab('lh'), i - 1] +
    sim[rev(z.lab('dxr')), i] *
      (sim[paste0(z.lab('b_s_'), rev(zlabs)), i - 1] +
        sim[paste0(z.lab('e_s_'), rev(zlabs)), i - 1])

  # Auxiliary Matrix with Household Demand for Shares
  e_h.matrix <- matrix(
    sim[
      apply(expand.grid(z.lab('e_h'), zlabs), 1, paste, collapse = "_"),
      i - 1
    ],
    nrow = N,
    ncol = N
  ) # creates a matrix "Z(row)_e_h_Z(column)"

  # Auxiliary Dividends Term
  sim[z.lab('div_h1'), i] <- (1 - parms[z.lab('omega')]) *
    (sim[z.lab('yn'), i] -
      sim[z.lab('wb'), i] -
      sim[z.lab('af'), i] -
      sim[z.lab('rl'), i - 1] * sim[z.lab('lf'), i - 1])

  # Disposable Income of Domestic Households 2
  if (sum(sim[z.lab('e_s'), i - 1] == 0) > 0) {
    sim[z.lab('yd'), i] <- sim[z.lab('yd'), i] + sim[z.lab('div_h1'), i]
  } else {
    sim[z.lab('yd'), i] <- sim[z.lab('yd'), i] +
      (x.r.mat * e_h.matrix) %*%
        (sim[z.lab('div_h1'), i] / sim[z.lab('e_s'), i - 1])
  }
  ...
  for (z in zlabs) {
    if (sim[paste0(z, '_', 'va'), i] > 0) {
      ...
      sim[paste0(z, '_yd_j-', 1:K), i] = sim[paste0(z, '_yd'), i] *
        sim[paste0(z, '_va_j-', 1:K), i] /
        sim[paste0(z, '_va'), i]

      # Disposable income of workers by industry (j=1,2,3,4,5)
      sim[paste0(z, '_ydw_j-', 1:K), i] = sim[paste0(z, '_wb_j-', 1:K), i] *
        (1 - sim[paste0(z, '_theta_w'), i])

      # Disposable income of capitalists by industry
      sim[paste0(z, '_ydc_j-', 1:K), i] = sim[paste0(z, '_yd_j-', 1:K), i] -
        sim[paste0(z, '_ydw_j-', 1:K), i]
      ...
      sim[paste0(z, '_ineq'), i] = abs(
        1 - sim[paste0(z, '_ydw'), i] / sim[paste0(z, '_ydc'), i]
      )
      sim[paste0(z, '_ineq_j-', 1:K), i] <- abs(
        1 -
          sim[paste0(z, '_ydw_j-', 1:K), i] / sim[paste0(z, '_ydc_j-', 1:K), i]
      )

      # Shares of Disposable Income
      sim[paste0(z, '_shp'), i] <- sim[paste0(z, '_ydc'), i] /
        sim[paste0(z, '_yd'), i]
      sim[paste0(z, '_shw'), i] <- sim[paste0(z, '_ydw'), i] /
        sim[paste0(z, '_yd'), i]
      sim[paste0(z, '_shp_j-', 1:K), i] <- sim[paste0(z, '_ydc_j-', 1:K), i] /
        sim[paste0(z, '_yd_j-', 1:K), i]
      sim[paste0(z, '_shw_j-', 1:K), i] <- sim[paste0(z, '_ydw_j-', 1:K), i] /
        sim[paste0(z, '_yd_j-', 1:K), i]
    }
  }

  # Net Household Wealth
  sim[z.lab('v'), i] = sim[z.lab('v'), i - 1] +
    sim[z.lab('yd'), i] -
    sim[z.lab('c'), i] * sim[z.lab('pa'), i]

  #### PRODUCTION FIRMS ####

  # ── FINAL DEMAND VECTOR (MRIO-consistent) ─────────────────────────────────
  # Identical to MVP_model_2026.R: eta_other*rex encodes off-diagonal (Z21, Z12)
  # blocks; eta*imp removes the leakage. The FD_MRIO sheet in the initial state
  # provides the bilateral channel breakdown for accounting/analysis but does not
  # alter the aggregate dynamics.
  ...
  sim[z.lab('yn'), i] = sim[z.lab('c'), i] *
    sim[z.lab('pa'), i] +
    sim[z.lab('id'), i] * sim[z.lab('pid'), i] +
    sim[z.lab('id_g'), i] * sim[z.lab('pid_g'), i] +
    sim[z.lab('g'), i] * sim[z.lab('pg'), i] +
    sim[z.lab('nex'), i] -
    sim[z.lab('nimp'), i] -
    sim[z.lab('vat_rev'), i] -
    sim[rev(z.lab('xr')), i] * sim[rev(z.lab('tar_rev')), i]

  # Corporate Profit
  sim[z.lab('f_f'), i] = sim[z.lab('yn'), i] -
    sim[z.lab('wb'), i] -
    sim[z.lab('af'), i] -
    sim[z.lab('rl'), i - 1] * sim[z.lab('lf'), i - 1]
  ...
  sim[zk.lab('va_j'), i] <- sim[zk.lab('p'), i] *
    sim[zk.lab('x'), i] -
    sim[zk.lab('p'), i] %*% A.xr * sim[zk.lab('x'), i]
  ...
  sim[zk.lab('rva_j'), i] <- sim[zk.lab('p'), i - 1] *
    sim[zk.lab('x'), i] -
    sim[zk.lab('p'), i - 1] %*% A.xr * sim[zk.lab('x'), i]
  ...
  sim[z.lab('va_index'), i] <- sim[z.lab('va_index'), i - 1] *
    (1 + sim[z.lab('va_g'), i])
  ...
  sim[z.lab('id'), i] = parms[z.lab('gamma')] *
    (sim[z.lab('kt'), i] - sim[z.lab('k'), i - 1]) +
    sim[z.lab('da'), i] +
    sim[z.lab('id_p0'), i]
  ...
  # Fixed Capital Stock
  sim[z.lab('k'), i] = sim[z.lab('k'), i - 1] +
    sim[z.lab('id'), i] -
    sim[z.lab('da'), i]

  # Amortization Funds
  sim[z.lab('af'), i] = sim[z.lab('da'), i] *
    sim[z.lab('pid'), i - 1] -
    sim[z.lab('k'), i] * (sim[z.lab('pid'), i] - sim[z.lab('pid'), i - 1])

  # Corporate Demand for Bank Loans
  sim[z.lab('lf'), i] = sim[z.lab('lf'), i - 1] +
    sim[z.lab('id'), i] * sim[z.lab('pid'), i] -
    sim[z.lab('af'), i] -
    sim[z.lab('f_f_u'), i] -
    (sim[z.lab('e_s'), i] - sim[z.lab('e_s'), i - 1]) #NEW

  # Quantity of Shares Issued by Private Firms
  sim[z.lab('e_s'), i] = sim[z.lab(paste0('e_h_', zlabs)), i] +
    sim[rev(z.lab('xr')), i] * sim[rev(z.lab(paste0('e_h_', rev(zlabs)))), i]

  # Supply of Shares to Households
  sim[z.lab(paste0('e_s_', rev(zlabs))), i] = sim[
    z.lab(paste0('e_h_', rev(zlabs))),
    i
  ] *
    sim[z.lab('xr'), i] #NEW

  # Percentage Return Rate on Shares Issued by Private Firms
  sim[z.lab('r_e'), i] = (1 - parms[z.lab('omega')]) *
    sim[z.lab('f_f'), i] /
    sim[z.lab('e_s'), i] #NEW
  ...
  sim[z.lab('div_h1'), i] <- (1 - parms[z.lab('omega')]) *
    (sim[z.lab('yn'), i] -
      ...
      sim[z.lab('rl'), i - 1] * sim[z.lab('lf'), i - 1])
  if (sum(sim[z.lab('e_s'), i - 1] == 0) > 0) {
    sim[z.lab('div'), i] <- sim[z.lab('div_h1'), i]
  } else {
    sim[z.lab('div'), i] <- e_h.matrix %*%
      (sim[z.lab('div_h1'), i] / sim[z.lab('e_s'), i - 1])
  }
  ...
  for (z in zlabs) {
    if (sim[paste0(z, '_ms'), i] > sim[paste0(z, '_ls'), i]) {
      sim[paste0(z, '_b_b'), i] = sim[paste0(z, '_ms'), i] -
        sim[paste0(z, '_ls'), i]
      sim[paste0(z, '_a_d'), i] = 0
    } else {
      sim[paste0(z, '_a_d'), i] = sim[paste0(z, '_ls'), i] -
        sim[paste0(z, '_ms'), i]
  ...
  # Bank Profit
  sim[z.lab('f_b'), i] = sim[z.lab('rl'), i - 1] *
    sim[z.lab('lf'), i - 1] +
    sim[z.lab('rb'), i - 1] * sim[z.lab('b_b'), i - 1] -
    sim[z.lab('rm'), i - 1] * sim[z.lab('ms'), i - 1] +
    sim[z.lab('rh'), i - 1] * sim[z.lab('lh'), i - 1]

  #### GOVERNMENT AND CENTRAL BANK ####

  # Government Spending
  sim[z.lab('g'), i] = sim[z.lab('g'), i - 1] *
    (1 + parms[z.lab('g_g')]) +
    sim[z.lab('g0'), i]
  ...
  sim[z.lab('k_g'), i] = sim[z.lab('k_g'), i - 1] +
    sim[z.lab('id_g'), i] -
    sim[z.lab('da_g'), i]

  # Income Tax Payments
  sim[z.lab('t'), i] = sim[z.lab('theta_w'), i] *
    sim[z.lab('wb'), i] +
    sim[z.lab('theta_c'), i] *
      (sim[z.lab('div'), i] +
        sim[z.lab('rb'), i - 1] * sim[paste0(zlabs, '_b_s_', zlabs), i - 1] +
        sim[z.lab('rm'), i - 1] * sim[z.lab('mh'), i - 1])
  sim['Z1_t', i] = sim['Z1_t', i] +
    sim['Z1_theta_c', i] *
      sim['Z2_rb', i - 1] *
      sim['Z1_b_s_Z2', i - 1] *
      sim['Z2_xr', i - 1]
  sim['Z2_t', i] = sim['Z2_t', i] +
    sim['Z2_theta_c', i] *
      sim['Z1_rb', i - 1] *
      sim['Z2_b_s_Z1', i - 1] *
      sim['Z2_xr', i - 1]
  ...
  sim[zk.lab('t_j'), i] <- sim[zk.lab('va_j'), i] *
    rep(sim[z.lab('t'), i] / sim[z.lab('va'), i], each = K)

  # Interest Payments on Bills by Industry
  sim[zk.lab('int_j'), i] <- sim[zk.lab('va_j'), i] -
    sim[zk.lab('yd_j'), i] -
    sim[zk.lab('t_j'), i]
  ...
  sim[z.lab('tar_rev'), i] <- sim[rev(z.lab('xr')), i] *
    sim[z.lab('tot_imp'), i] *
    zk.sum(sim[zk.lab('tar'), i] * sim[zk.lab('eta'), i])
  # zk.sum(sim[rev.zk.lab('p'), i] * sim[zk.lab('tar'), i] * sim[zk.lab('eta'), i] / (1 + sim[zk.lab('tar'), i]))

  # Government Deficit #NEW
  sim[z.lab('gdef'), i] = sim[z.lab('g'), i] *
    sim[z.lab('pg'), i] -
    sim[z.lab('t'), i] +
    sim[z.lab('rb'), i - 1] * sim[z.lab('b_s'), i - 1] -
    sim[z.lab('f_cb'), i] -
    sim[z.lab('vat_rev'), i] -
    sim[z.lab('tar_rev'), i] +
    sim[z.lab('id_g'), i] * sim[z.lab('pid_g'), i]
  ...
  sim['Z1_f_cb', i] <- sim['Z1_f_cb', i] +
    sim['Z2_rb', i - 1] * sim['Z1_b_cb_s_Z2', i - 1] * sim['Z2_xr', i - 1] # Z1 has World Money
  ...
  sim['Z1_b_h_Z1', i] <- parms['Z1_lambda10'] *
    sim['Z1_v', i] + # Z1's domestic households demand for Z1 bills
    parms['Z1_lambda11'] * sim['Z1_rb', i - 1] * sim['Z1_v', i] -
    parms['Z1_lambda12'] *
      (sim['Z2_rb', i - 1] +
        (sim['Z2_xr', i] - sim['Z2_xr', i - 1]) / sim['Z2_xr', i - 1]) *
      sim['Z1_v', i] -
    parms['Z1_lambda13'] * sim['Z1_rm', i - 1] * sim['Z1_v', i] -
    parms['Z1_lambda14'] * sim['Z1_yd', i]

  sim['Z1_b_h_Z2', i] <- parms['Z1_lambda20'] *
    sim['Z1_v', i] - # Z1's domestic households demand for Z2 bills
  ...
  sim['Z2_b_h_Z2', i] <- parms['Z2_lambda10'] *
    sim['Z2_v', i] - # Z2's domestic households demand for Z2 bills
    parms['Z2_lambda11'] *
      (sim['Z1_rb', i - 1] +
        (sim['Z1_xr', i] - sim['Z1_xr', i - 1]) / sim['Z1_xr', i - 1]) *
      sim['Z2_v', i] +
    parms['Z2_lambda12'] * sim['Z2_rb', i - 1] * sim['Z2_v', i] -
    parms['Z2_lambda13'] * sim['Z2_rm', i - 1] * sim['Z2_v', i] -
    parms['Z2_lambda14'] * sim['Z2_yd', i]

  sim['Z2_b_h_Z1', i] <- parms['Z2_lambda20'] *
    sim['Z2_v', i] + # Z2's domestic households demand for Z1 bills
    parms['Z2_lambda21'] *
      (sim['Z1_rb', i - 1] +
        (sim['Z1_xr', i] - sim['Z1_xr', i - 1]) / sim['Z1_xr', i - 1]) *
      sim['Z2_v', i] -
  ...
  sim[z.lab('h_h'), i] <- parms[z.lab('lambdac')] *
    sim[z.lab('c'), i - 1] *
    sim[z.lab('pa'), i - 1]

  # Domestic Households Demand for Deposits
  sim[z.lab('mh'), i] <- sim[z.lab('v'), i] -
    sim[paste0(z.lab('b_h_'), zlabs), i] -
    sim[z.lab('h_h'), i] -
    sim[paste0(z.lab('b_h_'), rev(zlabs)), i] -
    sim[z.lab('e_h_Z1'), i] -
    sim[z.lab('e_h_Z2'), i] +
    sim[z.lab('lh'), i]
  ...
  for (z in zlabs) {
    sim[paste0(z, '_lh'), i] <- sim[paste0(z, '_lh'), i - 1] *
      (1 - parms[paste0(z, '_rep')]) +
      max(
        sim[paste0(z, '_c'), i] *
          sim[paste0(z, '_pa'), i] -
          sim[paste0(z, '_yd'), i],
        parms[paste0(z, '_psi')] *
          (sum(sim[paste0(z, '_p-', 1:K), i] * sim[paste0(z, '_dc-', 1:K), i]) -
            sum(
              sim[paste0(z, '_p-', 1:K), i - 1] *
                sim[paste0(z, '_dc-', 1:K), i - 1]
            ))
      )
  }
  ...
  p.t.a.aux <- p.t.a.aux *
    ...
    sim[zk.lab('w'), i] / sim[zk.lab('pr'), i]
  sim[zk.lab('p_t'), i] <- p.t.a.aux /
    (1 - diag(A) * (1 + sim[zk.lab('mu'), i]))
  ...
  sim[zk.lab('p'), i] <- (sim[zk.lab('p_t'), i] +
    sim[zk.lab('gamma_x'), i] *
      (sim[zk.lab('x'), i] - sim[zk.lab('x_star'), i - 1])) *
    (1 + sim[zk.lab('vat'), i])
  ...
  sim[z.lab('pim'), i] <- zk.sum(
    sim[rev.zk.lab('p'), i] *
      (1 + sim[rev.zk.lab('tar'), i] - sim[zk.lab('vat'), i]) *
      sim[zk.lab('eta'), i]
  ) *
    sim[rev(z.lab('xr')), i]
  ...
  sim[zk.lab('was_j'), i] <- sim[zk.lab('wa_j'), i - 1] +
    sim[zk.lab('x'), i] * sim[zk.lab('zeta'), i] -
    sim[zk.lab('x'), i] * colSums(A[seq(1, N) * K, ]) # (A[5, ] + A[10, ])
  ...
  sim[zk.lab('wa_j'), i] <- sim[zk.lab('x'), i] *
    sim[zk.lab('zeta'), i] -
    sim[zk.lab('x'), i] * colSums(A[seq(1, N) * K, ]) # (A[5, ] + A[10, ])
  ...
  sim[zk.lab('emis_j'), i] <- sim[zk.lab('x'), i] * sim[zk.lab('eps'), i]
  sim[zk.lab('land_j'), i] <- sim[zk.lab('x'), i] * sim[zk.lab('lambda_j'), i]
  sim[zk.lab('water_j'), i] <- sim[zk.lab('x'), i] * sim[zk.lab('omega_j'), i]

  # Annual CO2 Emissions of the Domestic Economy --- CHANGED ON 08/08/2023 <--- JBF
  sim[z.lab('emis'), i] <- zk.sum(sim[zk.lab('emis_j'), i])
  sim[z.lab('land'), i] <- zk.sum(sim[zk.lab('land_j'), i])
  sim[z.lab('water'), i] <- zk.sum(sim[zk.lab('water_j'), i])

  # C02 concentration using alternative method: Cumulative CO2 Emissions
  sim[z.lab('co2_cum'), i] <- sim[z.lab('co2_cum'), i - 1] +
    sim[z.lab('emis'), i]

  # Extraction of Matter
  sim[z.lab('x_mat'), i] <- zk.sum(
    sim[zk.lab('x'), i] * sim[zk.lab('mu_mat'), i]
  ) # Production of Material Goods
  sim[z.lab('mat'), i] <- sim[z.lab('x_mat'), i] - sim[z.lab('rec'), i] # Extraction of Matter
  sim[z.lab('rec'), i] <- sim[z.lab('rho_dis'), i] *
    sim[z.lab('dis'), i] + # Recycled Matter (of Socioeconomic Stock + Industrial Waste)
    sim[z.lab('mu_mat-5'), i] * sim[z.lab('x-5'), i]
  sim[z.lab('dis'), i] <- zk.sum(
    sim[zk.lab('mu_mat'), i] * # Discarded Socioeconomic Stock
      sim[zk.lab('zeta_dc'), i - 1] *
      sim[zk.lab('dc'), i - 1]
  )
  ...
  sim[z.lab('ren'), i] <- zk.sum(
    sim[zk.lab('eps_en'), i] * # Renewable Energy at the End of the Period
      sim[zk.lab('eta_en'), i] *
      sim[zk.lab('x'), i]
  )
  ...
  sim['kmat', i] <- sim['kmat', i - 1] +
    sim['conv_mat', i] - # Global Stock of Material Reserves
    sum(sim[z.lab('mat'), i])
  ...
  for (z in 1:length(zlabs)) {
    ...
    sim[paste0('Z', z, '_M_TOT_int'), i] <- sum(
      A[, foo] %*% sim[zk.lab('x'), i][foo]
    ) -
      sum(A[foo, foo] %*% sim[zk.lab('x'), i][foo])
  }

  # Real Final Imports
  if (t < 10) {
    sim[z.lab('imp'), i] <- ifelse(
      sim[z.lab('yd'), i - 1],
      exp(
        parms[z.lab('mu0')] + parms[z.lab('mu2')] * log(sim[z.lab('yd'), i - 1])
      ),
      0
    )
  } else {
    sim[z.lab('imp'), i] = exp(
      parms[z.lab('mu0')] -
        parms[z.lab('mu1')] *
          (log(sim[z.lab('pim'), i - 1]) - log(sim[z.lab('pa'), i - 1])) +
        parms[z.lab('mu2')] *
          log(sim[z.lab('yd'), i - 1] / sim[z.lab('pa'), i - 1])
    )
  }
  ...
  sim[z.lab('cab'), i] <- sim[z.lab('tb'), i] +
    sim[rev(z.lab('rb')), i - 1] *
      sim[paste0(z.lab('b_s'), '_', rev(zlabs)), i - 1] *
      sim[rev(z.lab('xr')), i - 1] -
    sim[z.lab('rb'), i - 1] *
      sim[paste0(rev(z.lab('b_s')), '_', zlabs), i - 1] +
    c(1, -1) *
      sim['Z2_rb', i - 1] *
      sim['Z1_b_cb_s_Z2', i - 1] *
      c(sim['Z2_xr', i - 1], 1)
  if (!sum(sim[z.lab('e_s'), i - 1] == 0)) {
    sim[z.lab('cab'), i] <- sim[z.lab('cab'), i] +
      sim[rev(z.lab('xr')), i] *
        (1 - parms[rev(z.lab('omega'))]) *
        sim[rev(z.lab('f_f')), i] *
        sim[paste0(z.lab('e_s'), '_', rev(zlabs)), i - 1] /
        sim[rev(z.lab('e_s')), i - 1] -
      (1 - parms[z.lab('omega')]) *
        sim[z.lab('f_f'), i] *
        sim[paste0(rev(z.lab('e_s')), '_', zlabs), i - 1] /
        sim[z.lab('e_s'), i - 1]
  }

  # Financial Account Balance, Net of Official Transactions
  sim[z.lab('kabp'), i] <- sim[paste0(rev(z.lab('b_s')), '_', zlabs), i] -
    sim[paste0(rev(z.lab('b_s')), '_', zlabs), i - 1] -
    (sim[paste0(z.lab('b_s'), '_', rev(zlabs)), i] -
      sim[paste0(z.lab('b_s'), '_', rev(zlabs)), i - 1]) *
      sim[rev(z.lab('xr')), i] +
    sim[paste0(rev(z.lab('e_s')), '_', zlabs), i] -
    sim[paste0(rev(z.lab('e_s')), '_', zlabs), i - 1] -
    (sim[paste0(z.lab('e_s'), '_', rev(zlabs)), i] -
      sim[paste0(z.lab('e_s'), '_', rev(zlabs)), i - 1]) *
      sim[rev(z.lab('xr')), i]
  ...
  sim['Z1_b_cb', i] <- sim['Z1_b_cb', i - 1] +
    (sim['Z1_h_s', i] - sim['Z1_h_s', i - 1]) -
    (sim['Z1_a_s', i] - sim['Z1_a_s', i - 1]) -
    (sim['Z1_b_cb_s_Z2', i] - sim['Z1_b_cb_s_Z2', i - 1]) * sim['Z2_xr', i] -
    (sim['Z1_or', i] - sim['Z1_or', i - 1]) * sim['Z1_p_or', i]

  # Z2 CB holdings of bills ------------------------------------ 12.83 --- CHANGED ON 04/08/2023 (ADVANCES ADDED)
  sim['Z2_b_cb', i] <- sim['Z2_h_s', i] -
    sim['Z2_or', i] * sim['Z2_p_or', i] -
    sim['Z2_a_s', i]
  ...
  sim['Z1_b_cb_s_Z2', i] = sim['Z2_b_s', i] -
    sim['Z1_b_s_Z2', i] -
    sim['Z2_b_s_Z2', i] -
    sim['Z2_b_cb', i] -
    sim['Z2_b_b', i]
  ...
  if (t <= 75) {
    sim[zk.lab('pop_j'), i] <- sim[zk.lab('n_j'), i]
  } else {
    sim[zk.lab('pop_j'), i] <- sim[zk.lab('pop_j'), i - 1] *
      (1 + sim[zk.lab('g_pop'), i]) +
      sim[zk.lab('imm'), i] -
      sim[rev.zk.lab('imm'), i]
  }
  ...
  # Immigration
  sim[zk.lab('imm'), i] <- sim[zk.lab('gamma_imm_0'), i] *
    sim[zk.lab('pop_j'), i - 1] +
    sim[zk.lab('gamma_imm_1'), i] * sim[zk.lab('un'), i - 1] +
    sim[zk.lab('gamma_imm_2'), i] *
      (sim[zk.lab('w'), i] - sim[zk.lab('w'), i - 1])

  # Female Employment Share by Industry
  sim[zk.lab('rho'), i] <- sim[zk.lab('parw0'), i] -
    sim[zk.lab('parw1'), i] * (sim[zk.lab('w'), i] - sim[zk.lab('w'), i - 1])
