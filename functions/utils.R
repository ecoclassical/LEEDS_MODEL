...
view.shock <- function(res, shock.vars, shock.sectors, shock.title) {
  ...
  df$name <- setNames(initial$vars$name, initial$vars$label)[as.character(
    df$var
  )]
  df$industry <- setNames(shock.sectors, shock.vars)[df$variable]
  df$full.name <- sapply(df$name, insert.line.break)
  df$full.name[!is.na(df$industry)] <- paste0(
    df$name[!is.na(df$industry)],
    ',\n',
